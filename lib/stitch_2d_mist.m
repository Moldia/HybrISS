...
for s = s_order
    ...
    imfile = files{s};
    %output_prefix=['Base_',num2str(s)];
    %output_prefix=cellstr(files(s_order));
    xypos = csvread(fullfile(outdir_top, '2DTiles', ['tile_coordinates_200311_dRNA_01_rd1cy1001.csv']));
    ...
    if ref_file && s~=ref_file
       ...
       output_prefix_ref=['Base_',num2str(s)];
